...
const busboy = require("busboy"); // ✅ FIX: niente destructuring
...
async function uploadFileToBlob(fileStream, projectId, originalName) {
  const blobName = `${projectId}/${originalName}`;
  const blockBlobClient = containerClient.getBlockBlobClient(blobName);
  await blockBlobClient.uploadStream(fileStream);
  return blockBlobClient.url;
}

app.http("CreateProjectFiles", {
  ...
  handler: async (req, context) => {
    ...
    try {
      ...
      // ✅ FIX: busboy è la funzione diretta, + Object.fromEntries per Azure v4
      ...
      // ✅ FIX: terzo argomento è `info` (oggetto), non `filename` (stringa)
      bb.on("file", (fieldname, file, info) => {
        const { filename } = info;
        const pass = new stream.PassThrough();
        file.pipe(pass);
        uploadedBlobs.push({ stream: pass, filename });
      });
      ...
      await new Promise((resolve, reject) => {
        ...
        // ✅ FIX: req non è uno stream Node.js in Azure Functions v4
        req
          .arrayBuffer()
          .then((buffer) => {
      ...
      const uploadedUrls = [];
      try {
        for (const file of uploadedBlobs) {
